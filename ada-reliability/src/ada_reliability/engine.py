...
class AdaEngine:
    ...
    def _stored_passport(self, passport: Optional[dict], agent_id: str, task_type: str) -> tuple[Optional[dict], Optional[str]]:
        ...
        if stored["task_type"] not in (task_type, "*") and passport.get("task_type") not in (task_type, "*"):
    ...
    def authorize(self, *, action: str, payload: dict, passport: Optional[dict],
        ...
        if tool["requires_receipt"]:
            ...
            if mutation_type and classes and mutation_type not in classes and "*" not in classes:
        ...
        if decision == "ALLOW":
            grant_id = f"{context_receipt}:{sha256_obj(payload or {})}:{action}:{site_id}"
    ...
    def journal_intent(self, *, task_run_id: str, idempotency_key: str, tool_name: str,
        ...
        for g in self._authz_grants.values():
            if (g["decision"] == "ALLOW"
                ...
                break
        if grant is None:
            raise AdaError("NOT_AUTHORIZED", "journal requires matching ALLOW for this payload/tool/site")

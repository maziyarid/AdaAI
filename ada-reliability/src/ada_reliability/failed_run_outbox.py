...
class FailedRunOutbox:
    ...
    def record_failure(
        ...
        *,
        ...
        lifecycle = _lifecycle_for(classified)
        next_retry = None
        if lifecycle != LIFECYCLE_PARKED:
        ...
        rec = {
            ...
            "reset_condition": reset_condition or DEFAULT_RESET.get(classified),
            "external_sync_state": (
    ...
    def claim_batch(
        ...
        for rec in self.eligible(now):
            ...
            rec["lease_until"] = (now + timedelta(seconds=self.lease_seconds)).isoformat()
            stored = self.store.put(rec)
            self.store.append_event(stored["id"], "claimed", {"owner": owner})
    ...
    def replay_one(self, rec: dict) -> dict:
        ...
        if kind in PRODUCTION_MUTATION_KINDS:
            ...
            if kind in ("wordpress_mutation", "packet_ack"):
                ...
                if outcome.get("status") == "APPLIED":
                    ...
                    return self._succeed(rec, "engine_applied")
                if outcome.get("status") == "DUPLICATE_SKIPPED":
                    self.store.mutation_record(kind, key)
                    return self._succeed(rec, "duplicate_mutation_skipped", skipped=True)
                return self._requeue(rec, outcome.get("status") or "ENGINE_RETRY")
        ...
        # Transient non-mutation failure: evidence preserved; mark retryable
        # work complete only when a supplied engine retry says so.

...
A legacy Agiflow row that has already been durably handed off to control-core pending_external_sync is a different case: after independently verifying the exact bridge stable ID (agiflow:<legacy stable_id>) and idempotency key in control-core, pass --external-sync-delegation STABLE_ID=agiflow:STABLE_ID. The planner then preserves that row as parked historical evidence with mutation_kind=none; control-core external-sync remains the sole delivery authority. Never provide both a durable job binding and an external-sync delegation for one row.

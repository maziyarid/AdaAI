...
CREATE TABLE IF NOT EXISTS memory_links (
  source_id uuid NOT NULL REFERENCES memory_records(id) ON DELETE CASCADE,
  target_id uuid NOT NULL REFERENCES memory_records(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS task_runs (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  parent_task_run_id uuid REFERENCES task_runs(id),
...
CREATE INDEX IF NOT EXISTS idx_task_parent ON task_runs(parent_task_run_id);
...
CREATE TABLE IF NOT EXISTS context_receipts (
  ...
  qalam_release text,
  qalam_hash text,
  passport_id uuid REFERENCES agent_passports(id),
...
CREATE TABLE IF NOT EXISTS approval_tickets (
  ...
  payload_hash text NOT NULL,
  snapshot_hash text,
  context_receipt_id uuid NOT NULL REFERENCES context_receipts(id),
...
CREATE TABLE IF NOT EXISTS approval_events (
  id bigserial PRIMARY KEY,
  approval_ticket_id uuid NOT NULL REFERENCES approval_tickets(id) ON DELETE CASCADE,
  event_type text NOT NULL,
  actor text NOT NULL,
...
CREATE TABLE IF NOT EXISTS model_registry (
  ...
  weights_memory_mb integer,
  peak_memory_4k_mb integer,
  peak_memory_16k_mb integer,
  peak_memory_32k_mb integer,
  peak_memory_c1_mb integer,
  peak_memory_c4_mb integer,
  tokens_per_second numeric,
  tool_call_success_rate numeric,
  structured_output_failure_rate numeric,
...
-- Initialize the global scope; other scopes are created lazily at bootstrap.
